...
CHROMATIC_SCALE_PITCH_CLASSES = {*[i in range(0,11)]}

C_major = [0] + sorted(filter(lambda x: (x % 12) in \
    MAJ_SCALE_PITCH_CLASSES, range(1, NOTE_MAX)))
Db_major = list(map(lambda x: x+1, C_major))
D_major = list(map(lambda x: x+2, C_major))
Eb_major = list(map(lambda x: x+3, C_major))
E_major = list(map(lambda x: x+4, C_major))
F_major = list(map(lambda x: x+5, C_major))
Gb_major = list(map(lambda x: x+6, C_major))
G_major = list(map(lambda x: x+7, C_major))
Ab_major = list(map(lambda x: x+8, C_major))
A_major = list(map(lambda x: x+9, C_major))
Bb_major = list(map(lambda x: x+10, C_major))
B_major = list(map(lambda x: x+11, C_major))
